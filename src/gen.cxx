...
int main (int argc, char** argv) {
  ...
  if (argc < 7) {
    std::cout << " usage: gen BEAMA BEAMB SEED SQRTS MINPTHAT NEVT BOOST FILENAMEOUT" << std::endl;
    return 0;
  }
  ...
  const float ptHatMin = atof (argv[5]);
  const int nEvents = atoi (argv[6]);
  const float boost = atof (argv[7]);
  const string outFileName = string (argv[8]);

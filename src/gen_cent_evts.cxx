...
int main (int argc, char** argv) {
  ...
  outTree->Branch ("gap_negEta",  &gap_negEta,  "gap_negEta/F");
  outTree->Branch ("gap_posEta",  &gap_posEta,  "gap_posEta/F");
  ...
  for (int iEvent = 0; iEvent < nEvents; iEvent++) {
    ...
    for (int i = 0; i < 98; i++) nPartEta[i] = 0;
    ...
    for (int i = 0; i < pythia.event.size (); i++) {
      ...
      if (pythia.event[i].pT () > 0.2) {
        int iEta = 0;
        while (-4.9+0.1*(iEta+1) < std::fabs (pythia.event[i].eta ())) iEta++;
        if (iEta < 98)
          nPartEta[iEta]++;
      }
